...
class WindowManager():
    ...
    def load_dataset(self, neo_rawio_class=None, file_or_dir_names=[], io_params={}):
        ...
        file_path = filedialog.askopenfilename(
            title= "Please select the synchronized video file:",
            filetypes=(("video files","*.mp4"),("video files","*.avi"))
            )
        source =  MultiVideoFileSource([file_path])
        view = VideoViewer(source=source, name='video')
...
class RawNeoOpenDialog(QT.QDialog):
    ...
    def on_addfiles(self):
        ...
        if self.neo_rawio_class.rawmode.endswith('-file'):
            ...
            #~ fd.setNameFilters(['All (*)'])
            fd.setWindowTitle("Select folder (OpenEphys data folder name typically contains the date)")
